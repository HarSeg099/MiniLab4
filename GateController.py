...
class GateController:
    ...
    def stateDo(self, state):
        ...
        if state == 0:
            if self._gate.motionDetected() == True:
            ...
            # 	gotoState(2)
            if self._gate.vehiclePresent() == True:
    ...
    def stateEntered(self, state, event):
        ...
        if state == 0:
            self._gate.close()
            self._display.showText(f'Welcome to PICO ') 
        elif state == 1:
            self._gate.open()
            self._buzzer.beep()
            self._display.showText(f'Gate OPEN      ')
        elif state == 2:
            self._timer.start(5)
            ...
            self._gate.open()
            self._buzzer.beep()
            self._display.showText(f'Gate Open Care')
        elif state == 4:
            self._timer.start(2)
